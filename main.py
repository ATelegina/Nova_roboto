...
def libera (tabelo):
        if "" in tabelo.col_values(1):
            ...
            print("erar2")
        return libera
@bot.message_handler(commands=['porrobotuloj-voko'])
def porrobotuloj(message):
    porrobotula_list = [602309534, 1476873510, 602746876, 1036027959, 310873155, 1823897017, 184113675, 1877072611, 334163171, 714198737, 1715898586, 400550232, 366513807, 1643338288, 345452414, 2020594821]
    teksto = ""    
    for i in porrobotula_list:
        teksto = teksto + "<a href='tg://user?id={userid}'>@porrobotuloj</a>".format(userid=i)
    mesg = bot.send_message(ne_id, teksto, parse_mode="html")
    bot.edit_message_text(chat_id=ne_id, text="@porrobotuloj", message_id = mesg.id)
@bot.message_handler(commands=['atb_komenco'])
def sendu_atb_komenco(message):
  bot.send_message(ne_id, """Saluton el Anonymous Telegram Bot! Roboto de @LaiconBots!

Vi povas denun sendi anonimajn mesaĝojn en tiu grupo.
Nur babilante kun mi private… kion vi atendas?!?!
Por pluaj instruoj bonvolu tajpi la komandon /help

🎱🎱🎱 SPONSORO 🎱🎱🎱
Demandu ion ajn al tutaj nekonatuloj tute anonime per @social8ball_bot (http://t.me/social8ball_bot?start=AnonymousTelegramBot)""")      
@bot.callback_query_handler(func=lambda call: True)
def callback_query(call):
    if call.data == 'jes':
        certas_demando(call)
    elif call.data == 'raportu':
        bot.answer_callback_query(call.id, "Enhavo reportita (Nur la unua reporto por ĉiu uzanto estas kalkulita)")
            
@bot.message_handler(commands=["kloakanoj"])
def helikumu(message):
    admins_ids = [] 
# =============================================================================
#     a = bot.get_chat_administrators(message.chat.id)
#     for chat_member in a:
#         admins_ids.append(chat_member.user.id)
#     print("Jen: ", admins_ids)    
#     bot.send_message(message.chat.id, "<b><a href='tg://user?id={userid}'>{}</a> jonizulis. Forta krizo.</b>".format("lll", userid = 602309534), parse_mode="html")    
# =============================================================================
...
def chat_m(message: types.ChatMemberUpdated):
    ...
    if new.status == "member":
        bot.send_message(message.chat.id,'Bonvenon, nova kloakano! Mi estas roboto. Sendu al mi private mesaĝon, tajpu tie "Jes" kaj vidu magion💫')
                                        ...
                                        if i < -4:
                                            ...
                                            for uzanto in worksheet.col_values(1):
                                                if uzanto == str(fiuzanto):
                                                    kolomno = worksheet.col_values(1).index(str(fiuzanto)) + 1
                                                    worksheet.delete_row(kolomno)        
                                            if not atb_mode: bot.send_message(message.chat.id, "La uzanto estos silentigita dum unu horo")
                                            try:
                                                time.sleep(0.3)
                                                if not atb_mode: bot.delete_message(message.chat.id, message.reply_to_message.message_id)
                                            except Exception as e:
                                                pass
                                        else:
                                            if i > 0 : 
                                                if not atb_mode: bot.send_message(message.chat.id, "Karmo de la uzanto: +" + str(i))
                                            else: 
                                                if not atb_mode: bot.send_message(message.chat.id, "Karmo de la uzanto: " + str(i))
                    else:
                        bot.send_message(message.chat.id, "La mesaĝo estas tro malnova, mi ne scias, kiu sendis ĝin")
@bot.message_handler(commands=['plani', "planu"])
def planu_message(message):
    mesg = bot.send_message(message.chat.id, "Sendu mesaĝon kiun vi volas plani")
    if multe_da_grupoj:
        bot.register_next_step_handler(mesg, sendi_kie)
    else:
        bot.register_next_step_handler(mesg, send_rezulto)
def sendi_kie(message):
    pass
def send_rezulto(message):
    pass      
@bot.message_handler(commands=['redaktu', 'redakti'])
...
def sendu_helpon(message):
    if not atb_mode: bot.send_chat_action(message.chat.id, 'typing')
    time.sleep(1)
    if not atb_mode: bot.send_message(message.chat.id, """Mi estas Robotino, ina roboto kreita speciale por @Esperantujoo. Sendu al mi private mesaĝon, kaj, se vi estas vera kloakano, mi resendos ĝin al Nova Esperantujo.
<b>Komandoj:</b>
         ...
         if bot.get_chat_member(ne_id, message.from_user.id).status == 'left':   
             ...
             pass
                     ...
                     if atb_mode:
                         ...
                         itembtn1 = types.InlineKeyboardButton(text = 'Nova Esperantujo | Esperanto', callback_data="jes")
                         markup.add(itembtn1)
                         bot.send_message(message.chat.id, """En kiun inter la sekvaj grupoj vi volas sendi la mesaĝon?
Se la grupon, kiun vi volas, ne estas en la listo, aldonu min kiel membro aŭ se mi jam estas membro skribu mesaĝon en la grupo antaŭe""", reply_markup=markup)
            ...
            if message.reply_to_message:
                ...
                if cu_robotino == nomo_de_roboto:
                    ...
                    if int(message.reply_to_message.date) + 97200 > int(time.time()):
                                        ...
                                        if nombro == 0:
                                            if not atb_mode: bot.send_message(message.chat.id, "Ho, ĉu vi simpas?")
                                            return
                                        worksheet = sh.worksheet("karmo_b")
                                        numero = 0
                                        for ulo in worksheet.col_values(1):
                                            ...
                                            if ulo == str(bonulo):
                                                cu_jam = worksheet.cell(numero, 2).value
                                                if str(message.from_user.id) == str(cu_jam):
                                                    if not atb_mode: bot.send_message(message.chat.id, "Via simpado por ĉi tiu uzanto ne havas limojn")
                                                    return
                                        if "" in worksheet.col_values(1):
                                            liberec = worksheet.col_values(1).index("") + 1
                                        else:
                        ...
                        if not atb_mode: bot.send_message(message.chat.id, "La mesaĝo estas tro malnova, mi ne scias, kiu sendis ĝin")
                else:
                    idilo = message.reply_to_message.from_user.id
...
def certas_demando(message):
        try:
                 ...
                 if atb_mode:
                     pass
                 elif (cu.lower() == u'jes') or (cu.lower() == u'"jes"'):
                     user.cu = cu
                 elif cu.lower() == u'ne':
                     time.sleep(0.3)
                 ...
                 def gen_markup():
                     ...
                     markup.row_width = 1
                     markup.add(types.InlineKeyboardButton("❗️Reportu enhavon❗️", callback_data="raportu"))
                     return markup
                 ...
                 if atb_mode:
                     bot.send_message(message.chat.id, "Mesaĝo sendite")
                 else:
